prawn_document(:page_layout => :landscape) do |pdf|
 ...
 @campaigns.each do |campaign|
  ...
  pdf.table(table_data, :column_widths => [200, 75, 50, 75, 75, 125, 125], :cell_style => { inline_format: true, border_color: 'C0C0C0', padding: 3, align: :center, text_color: '5D5D5D' }) do
          self.row_colors = %w(FFFFFF 666666)
          row(0).background_color = 'D9D9D9'
          self.header = true
        end

    pdf.repeat :all do
         pdf.bounding_box [pdf.bounds.left, pdf.bounds.bottom + 25], :width  => pdf.bounds.width - 20 do
          pdf.stroke_horizontal_rule
          pdf.move_down(10)
          pdf.text "All Rights Reserved \u00AE Brand Reach | Copyright 2015", :size => 10, align: :center, color: '5D5D5D'
        end
    end
end